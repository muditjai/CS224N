...
def softmax(x):
    ...
    ### YOUR CODE HERE
    x_rowmax = tf.reduce_max(x, axis=1, keep_dims=True)
    x_minux_max = tf.subtract(x, x_rowmax)
    x_exp = tf.exp(x_minux_max)
    x_exp_sum = tf.reduce_sum(x_exp, axis=1, keep_dims=True)
    out = tf.div(x_exp, x_exp_sum)
    # out = tf.Print(out, [x, x_rowmax, x_minux_max, x_exp, x_exp_sum, out], summarize=10)
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    log_y_hat = tf.log(yhat + 1e-10)
    out = -tf.reduce_sum(tf.to_float(y) * log_y_hat)
    ### END YOUR CODE

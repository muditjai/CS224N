...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embeddings_mat = tf.Variable(initial_value=self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(embeddings_mat, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        # Add an additional hidden layer for part 2i.
        q2i = False
        if q2i:
            W2 = tf.Variable(xavier_weight_init()((self.config.hidden_size, self.config.hidden_size)))
            b22 = tf.Variable(tf.zeros(self.config.hidden_size))
            z2 = tf.matmul(h_drop, W2) + b22
            h2 = tf.nn.relu(z2)
        else:
            h2 = h_drop
